			...
			if (continua == 's' || continua == 'S') {
				...
				do {
					...
					if (qtdTaloes <= 0 || qtdTaloes > 3) {
						System.out.println(
								"VOCÊ PODE ESCOLHER ENTRE 1 E 3 TALÕES DE CHEQUE. INSERIR UMA OUTRA QUANTIDADE? S/N:");
						continua = ler.next().charAt(0);
						if (continua != 's' && continua != 'S') { // sai do loop se o usuário não responder sim à
																	// pergunta anterior
							break;
						}
					}
	...
	static void menuContaEspecial(Scanner ler) {
		...
		for (int giro = 0; giro < 10; giro++) {
			do {
				...
				if (total < 0) {
					...
					System.out.printf(
							"Ok, seu saldo é %.2f. Deseja realizar outra operação na sua conta poupança? S/N ", total);
					continuar = ler.next().charAt(0);
		...
		while (recomecar == 'S' || recomecar == 's') {
			...
			if (soma > 1000) {
				...
				System.out.printf(
						"Ok, obrigada! Você está usando R$%.2f do seu limite especial, então seu saldo agora é de R$%.2f",
						limite, total + limite);
